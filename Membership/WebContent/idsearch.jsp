...
<c:if test="${ not (empty param.username) and not(empty param.email)}">
	<sql:setDataSource driver="oracle.jdbc.driver.OracleDriver" url="jdbc:oracle:thin:@192.168.56.4:1521:orcl"
...
<body>
	...
	<fieldset>
		...
		<ul>
			...
			<li>Email : <input type="text" name="email" size="50"></li>
			<li><input type="submit" value="검색"></li>
			<c:if test="${ not (empty param.username) and not(empty param.email)}">
				<c:if test="${rs.getRowCount() eq 0}">
					<li><span style="color:red"> 일치된 검색 결과가 없습니다. 확인 후 다시 입력해주세요.</span></li>	
				</c:if>
