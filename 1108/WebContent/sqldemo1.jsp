...
<c:catch var="error">
	<sql:setDataSource dataSource="jdbc/myoracle" var="conn"/><!-- DB연결 -->
...
<c:if test="${empty(error) }">
	...
	<head>
	<meta charset="UTF-8">
	<title>쌍용 주식 회사 사원명단</title>
	</head>
	<body>
	<h1 style="text-align:center; color:blue;">쌍용 주식 회사 사원명단</h1>
			...
			<tbody>
				<c:forEach items="${rs.rows}" var="row">
					<tr>
						...
						<td><fmt:formatDate type="date" pattern="yyyy-MM-dd" value="${row.hiredate}"/></td>
						<td>${row.dname}</td>
						<td>${row.loc}</td>
						<td>${row.deptno}</td>
					</tr>
