...
<sql:setDataSource driver="org.mariadb.jdbc.Driver" 
	...
	password="root" var="mydatasource" />
<sql:query dataSource="${mydatasource}" 	sql="SELECT * FROM city WHERE CountryCode = ?" var="rs">
	<sql:param value="KOR"/>
	</sql:query>
	<%-- ROW갯수 : ${rs.getRowCount()} --%>
	<h1 style="text-align:center;color:green;">한국의 도시 정보</h1>
	<table border="1" style="margin-left:auto;margin-right:auto;">
		<thead>
			<tr>
				<th>일련번호</th><th>도시명</th><th>지역</th><th>국가코드</th><th>도시인구</th>
			</tr>
		</thead>
		<tbody>
			<c:forEach items="${rs.getRows()}" var="myrow">
				<tr>
					<td>${myrow.ID}</td><td>${myrow.Name }</td><td>${myrow.District }</td>
					<td>${myrow.CountryCode }</td><td>${myrow.Population }</td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
