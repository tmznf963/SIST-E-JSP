<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="java.util.Vector, com.fnf.controller.Util" %>
...
<%
	...
	String sd = "C:/jsphome/.metadata/.plugins/org.eclipse.wst.server.core/tmp0/wtpwebapps/FNF/files/";
	int pageSize = 5;
	...
	<div id="wrapper">
		...
		<div id="sidebar-wrapper">
			<ul class="sidebar-nav">
				...
				<li><a href="#">[상품관리]</a></li>
				<li><a href="#">상품리스트[수정:삭제]</a></li>
				<li><a href="insertproduct.html">상품등록</a></li>
				<li><a href="#"><hr></a></li>
				<li><a href="#">[주문관리]</a></li>
                ...
                <div class="table-responsive">
                    <table class="table table-hover table1">
						...
						<c:forEach var="row" items="${vector}">
                                <tr>
                                    <td>
                                        <input name="ck_sub" type="checkbox" value="21"/>
                                        <input name="prd_prcseq" type="hidden" value="100"/>
                                    </td>
                                    <td>${row.getPcode()}</td>
                                    <td>${row.getPname()}</td>
                                    <td>
                                        <%-- <img src="<%=sd%>${row.getFilename()}" /> --%>
                                        <img src="img/${row.getFilename()}" width="50" height="75"/>
                                    </td>
                                    <td>${row.getCategory()}</td>
                                    <td class="text-right">
                                        ${row.getPrice()}
                                    </td>
                                    <td>${row.getSell_num()}</td>
                                    <td>${row.getWritedate()}</td>
                                    <td>
                                        <a href="productDetail.jsp?idx=${row.getIdx()}"><button class="btn btn-info btn-xs"  type="button" title="보기" >보기</button></a>
                                        <button class="btn btn-warning btn-xs"  type="button" title="수정" >수정</button>
                                        <button class="btn btn-danger btn-xs"  type="button" title="삭제" >삭제</button>
                                    </td>
                                </tr>
							</c:forEach>
						</c:if>
...
<!-- -------------------------------------------------------------------- -->
							...
							<div class="form-group">
								<div class="text-center">
									<div class="pageNum">
										<!-- * 처음이전처리 -->
										<c:if test="${currentPage eq 1}"><!-- 페이지가 1페이지 일 때 -->
											<button type="button" class="btn btn-info btn-sm">처음</button>
											<button type="button" class="btn btn-default btn-sm">이전</button>
										</c:if>
										<c:if test="${currentPage ne 1}"><!-- 페이지가 1페이지가 아닐 때 -->
											<a href="productList.jsp?page=${1}"><button type="button" class="btn btn-info btn-sm">처음</button></a>
											<a href="productList.jsp?page=${currentPage - 1}"><button type="button" class="btn btn-default btn-sm">이전</button></a>
										</c:if>
										<!-- 처음이전처리 -->
										
										<c:forEach begin="1" end="${pageCount}" var="i">
											<c:if test="${i eq currentPage}"><%--현재페이지 --%>
												<button class="btn btn-default btn-sm active" type="button">${i}</button>
											</c:if>
											<c:if test="${i ne currentPage}"><!-- 현재페이지가 아닌 -->
												<a href="productList.jsp?page=${i}"><button class="btn btn-default btn-sm" type="button">${i}</button></a>
											</c:if>
										</c:forEach>
										
										<!-- * 다음 끝처리 -->
										<c:if test="${currentPage eq pageCount}"><!-- 끝 페이지 일 때 -->
											<button type="button" class="btn btn-default btn-sm">다음</button>
											<button type="button" class="btn btn-info btn-sm">끝</button>
										</c:if>
										<c:if test="${currentPage ne pageCount}"><!-- 끝페이지가 아닐 때 -->
											<a href="productList.jsp?page=${currentPage + 1}"><button type="button" class="btn btn-default btn-sm">다음</button></a>
											<a href="productList.jsp?page=${pageCount}"><button type="button" class="btn btn-info btn-sm">끝</button></a>
										</c:if>
										<!-- 다음 끝처리 -->
									</div>
								</div>
